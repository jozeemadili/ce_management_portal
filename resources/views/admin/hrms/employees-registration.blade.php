						...
						<table class="table table-xs">
							<thead>
								...
								<tr>
                                    ...
                                    <th scope="col">Terms of_service</th>
							...
							<tbody>
								...
								<tr>
                                <!-- SELECT `id`, ``, ``, ``, ``, `email`, `first_appointmen_date`, `confirmation_date`, `retire_date`, `dob`, `gender`, `tribe`, `religion`, `domicile_place`, `nationality`, `nationality_type`, ``, ``, ``, ``, `marital_status`, `education_level`, `qualification`, ``, `status`, `reg_by`, `reg_date`, `pension_fund`, `company_id` FROM `employees` WHERE 1 -->
									<th scope="row">{{$loop->index + 1}}.</th>
									<td><a href="{{ Route('staff-profile', ['id' => $user->id])}}"><small>{{strtoupper($user->fname)}} {{strtoupper($user->mname)}} {{strtoupper($user->lname)}}</small></a></td>
                                   	<td>{{$user->emp_id}}</td>
                                    <td>{{$user->Directorate->dname}}</td>
                                    <td>{{$user->Section->sname}}</td>
                                    <td>{{$user->Branch->branch_name}}</td>
                                    <td>{{ number_format((float)$user->salary, 2, '.', ',') }} TZS</td>
                                    <td>{{$user->terms_of_service}}</td>
                                   	</tr>
                                @endforeach
							</tbody>
						</table>
    ...
    <div class="modal-dialog modal-fullscreen" role="document">
        <div class="modal-content">
            <div class="modal-header bg-primary text-white">
                <h5 class="modal-title">Employee Registration Form</h5>
                <button class="btn-close btn-close-white" type="button" data-bs-dismiss="modal" aria-label="Close" ></button>
            </div>
            <div class="modal-body">
            <div style="padding-right: 2em;padding-left: 2em;">
                        @livewire('hrms.registration-form')   
                    </div>
            </div>
        </div>
    </div>
