...
<body>
        ...
        <table>
            <tr>
                ...
                <td>
                    ...
                    <p>{{ optional($pledge->campaign)->name }}</p>
                </td>
            </tr>
        </table>
    </div>
    ...
    <div class="ref-box">
        <div class="ref">{{ $pledge->pledge_reference }}</div>
        <div class="status">{{ ucfirst(str_replace('_', ' ', $pledge->status)) }}</div>
    </div>

    <table class="details">
        <tr>
            <td class="label">Member</td>
            <td class="value">{{ optional($pledge->member)->first_name }} {{ optional($pledge->member)->last_name }}</td>
        </tr>
        <tr>
            <td class="label">Church</td>
            <td class="value">{{ optional(optional($pledge->member)->church)->name ?? '—' }}</td>
        </tr>
        <tr>
            <td class="label">Campaign</td>
            <td class="value">{{ optional($pledge->campaign)->name }}</td>
        </tr>
        <tr>
            <td class="label">Pledged Amount</td>
            <td class="value">{{ optional($pledge->campaign)->currency }} {{ number_format($pledge->amount, 2) }}</td>
        </tr>
        <tr>
            <td class="label">Fulfilled</td>
            <td class="value">{{ optional($pledge->campaign)->currency }} {{ number_format($pledge->totalFulfilled(), 2) }}</td>
        </tr>
        <tr>
            <td class="label">Outstanding</td>
            <td class="value">{{ optional($pledge->campaign)->currency }} {{ number_format($pledge->outstanding(), 2) }}</td>
        </tr>
        <tr>
            <td class="label">Frequency</td>
            <td class="value">{{ ucfirst(str_replace('_', ' ', $pledge->frequency)) }}</td>
        </tr>
        <tr>
            <td class="label">Pledged On</td>
            <td class="value">{{ optional($pledge->pledged_at)->format('d M Y') }}</td>
        </tr>
