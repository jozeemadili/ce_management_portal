...
<body>
    ...
    <table class="details">
        ...
        <tr>
            ...
            <td class="value">@if(optional($registration->program)->start_time){{ \Illuminate\Support\Carbon::parse($registration->program->start_time)->format('H:i') }}@endif @if(optional($registration->program)->start_time && optional($registration->program)->end_time)&ndash;@endif @if(optional($registration->program)->end_time){{ \Illuminate\Support\Carbon::parse($registration->program->end_time)->format('H:i') }}@endif</td>
        </tr>
        @endif
        <tr>
            <td class="label">Access</td>
            <td class="value">
                @if(optional($registration->program)->isFree())
                    FREE
                @else
                    PAID &middot; {{ optional($registration->program)->currency }} {{ number_format(optional($registration->program)->registration_fee, 2) }}
                @endif
            </td>
        </tr>
        <tr>
